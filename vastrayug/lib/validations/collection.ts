...
export const collectionSchema = z
  .object({
    ...
    slug: z
      ...
      .regex(SLUG_REGEX, 'Slug must be URL-safe (lowercase letters, numbers, hyphens)'),
    ...
    lifePathNumber: z
      .number()
      .int()
      .refine((n) => LIFE_PATH_NUMBERS.includes(n as (typeof LIFE_PATH_NUMBERS)[number]), {
  ...
  .refine(
    ...
    { message: 'Planet is required for Planetary collections', path: ['planet'] }
  )
  .refine(
    (data) => {
    ...
    { message: 'Zodiac sign is required for Zodiac collections', path: ['zodiacSign'] }
  )
  .refine(
    (data) => {
    ...
    { message: 'Life path number is required for Numerology collections', path: ['lifePathNumber'] }
  )

    ...
    >
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between">
            ...
            {links.map((link) => (
              <Link
                ...
                className={`text-sm tracking-wide font-body hover:text-nebula-gold transition-colors ${
                  pathname.startsWith(link.href) ? 'text-nebula-gold' : 'text-stardust-white/80'
                }`}
              >
                {link.label}
              </Link>
          ...
          {/* Logo */}
          <Link href="/" className="absolute left-1/2 -translate-x-1/2 text-2xl font-heading tracking-widest text-stardust-white flex items-center gap-2">
          ...
          <div className="flex items-center gap-4 md:gap-6 text-stardust-white">
            <button className="hover:text-nebula-gold transition-colors hidden md:block" aria-label="Search">
              <Search className="w-5 h-5" />
            </button>

            {wishlistEnabled && (
              <Link href="/account/wishlist" className="hover:text-nebula-gold transition-colors hidden md:block" aria-label="Wishlist">
            ...
            <div className="hidden md:block relative group">
              <Link href={session ? "/account" : "/login"} className="hover:text-nebula-gold transition-colors block">
              ...
              {session && (
                <div className="absolute right-0 mt-2 w-48 bg-deep-indigo border border-white/10 rounded-md shadow-cosmic opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                  <div className="py-1">
                    <Link href="/account" className="block px-4 py-2 text-sm text-stardust-white hover:bg-white/5">Profile</Link>
                    <Link href="/account/orders" className="block px-4 py-2 text-sm text-stardust-white hover:bg-white/5">Orders</Link>
                    <button onClick={() => signOut()} className="block w-full text-left px-4 py-2 text-sm text-mangal-red hover:bg-white/5">Logout</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <Link href="/cart" className="hover:text-nebula-gold transition-colors relative" aria-label="Cart">
        ...
        >
          ...
          <div className="mt-auto pt-8 border-t border-white/10">
            <div className="flex flex-col gap-4">
              {wishlistEnabled && (
                <Link href="/account/wishlist" className="flex items-center gap-3 text-stardust-white hover:text-nebula-gold">
                  <Heart className="w-5 h-5" />
                  <span>Wishlist</span>
                </Link>
              )}
              {session ? (
                <>
                  <Link href="/account" className="flex items-center gap-3 text-stardust-white hover:text-nebula-gold">
                    <User className="w-5 h-5" />
                    <span>My Account</span>
                  </Link>
                  <button onClick={() => signOut()} className="text-left text-mangal-red flex items-center gap-3 pt-2">
                     <span className="w-5 h-5" />
                     <span>Logout</span>
                  </button>
                </>
              ) : (
                <Link href="/login" className="flex items-center gap-3 text-stardust-white hover:text-nebula-gold">
                  <User className="w-5 h-5" />

...
async function main() {
  ...
  const planetaryCollections = [
    {
      ...
      description: 'Radiant garments channelling the sovereign energy of Surya — warmth, authority, and self-expression.',
    ...
    },
    {
      ...
      description: 'Ethereal pieces reflecting the nurturing glow of Chandra — intuition, calm, and emotional depth.',
      colourPaletteJson: { primary: '#F8F4EC', secondary: '#B8D4E3', accent: '#C9A84C' },
      sortOrder: 2,
    },
    {
      ...
      description: 'Structured designs embodying the discipline of Shani — resilience, justice, and karmic wisdom.',
      colourPaletteJson: { primary: '#191970', secondary: '#0A0A0F', accent: '#A8A9AD' },
      sortOrder: 3,
    },
    {
      ...
      description: 'Expansive silhouettes inspired by the wisdom of Guru — growth, abundance, and spiritual knowledge.',
      colourPaletteJson: { primary: '#FADA5E', secondary: '#C9A84C', accent: '#1B1640' },
      sortOrder: 4,
    },
  ]
  ...
  const vibeCollections = [
    {
      ...
      slug: 'the-sovereign',
      type: 'VIBE' as const,
      description: 'For those who lead with quiet confidence. Regal tones, structured fits, bold presence.',
      sortOrder: 5,
    },
    {
      name: 'The Healer',
      slug: 'the-healer',
      type: 'VIBE' as const,
      description: 'Gentle fabrics and calming hues for the nurturers. Comfort meets cosmic intention.',
      sortOrder: 6,
    },
    {
      name: 'The Wanderer',
      slug: 'the-wanderer',
      type: 'VIBE' as const,
      description: 'Free-spirited designs for the seekers. Movement, adventure, boundless expression.',
      sortOrder: 7,
    },
    {
      name: 'The Shadow',
      slug: 'the-shadow',
      type: 'VIBE' as const,
      description: 'Embrace the depth within. Dark palettes, mysterious textures, introspective elegance.',
      sortOrder: 8,
    },
    {
      name: 'The Reborn',
      slug: 'the-reborn',
      type: 'VIBE' as const,
      description: 'Transformation made visible. Fresh starts, evolving silhouettes, new cosmic chapter.',
      sortOrder: 9,
    },
  ]
  ...
  console.log(`✅ ${planetaryCollections.length + vibeCollections.length} collections seeded (${planetaryCollections.length} planetary + ${vibeCollections.length} vibe)`)
  ...
  const defaultSettings: { key: string; value: string }[] = [
    ...
    // Tab: Shipping
    {
      ...
      value: JSON.stringify({
        zones: [
          { name: 'All India', rate: 0, freeAbove: 999 },
        ],
        defaultRate: 99,
